...
import { Heart, Calendar, Send, Users, CheckCircle, XCircle } from 'lucide-react'
              ...
              <div className="space-y-4">
                <div className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-gray-800">Jeudi soir</span>
                    <Calendar className="w-5 h-5 text-gray-500" />
                  </div>
                  <p className="text-sm text-gray-600 mb-3">Dîner de répétition</p>
                  <div className="flex gap-3">
                    <button
                      type="button"
                      onClick={() => setAttendance({...attendance, thursday: true})}
                    ...
                    <button
                      type="button"
                      onClick={() => setAttendance({...attendance, thursday: false})}
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors ${
                ...
                <div className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-gray-800">Vendredi midi</span>
                    <Calendar className="w-5 h-5 text-gray-500" />
                  </div>
                  <p className="text-sm text-gray-600 mb-3">Cérémonie et réception</p>
                  <div className="flex gap-3">
                    <button
                      type="button"
                      onClick={() => setAttendance({...attendance, friday: true})}
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors ${
                    ...
                    <button
                      type="button"
                      onClick={() => setAttendance({...attendance, friday: false})}
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors ${
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-3xl font-bold text-blue-600">{thursdayCount}</div>
              <div className="text-sm text-blue-800">Jeudi soir</div>
            </div>
            <div className="text-center p-4 bg-purple-50 rounded-lg">
              <div className="text-3xl font-bold text-purple-600">{fridayCount}</div>
          ...
          <div className="space-y-3">
            {guests.map((guest) => (
              <div key={guest.id} className="border border-gray-200 rounded-lg p-4">
                ...
                {guest.message && (
                  <p className="text-gray-600 text-sm italic">"{guest.message}"</p>
                )}
              </div>
            ))}
          </div>
