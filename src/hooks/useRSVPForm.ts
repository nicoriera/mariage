...
import type { RSVPFormData, RSVPFormErrors, AttendanceState } from '../types/rsvp';
...
export function useRSVPForm(): UseRSVPFormReturn {
  ...
  const updateAttendance = useCallback((attendance: AttendanceState) => {
  ...
  const handleSubmit = useCallback(async (onSuccess?: (guest: Guest) => void) => {
      ...
      const result = await withDatabaseRetry(async () => {
        ...
        if (error) {
          if (error.message.includes('relation "public.guests" does not exist')) {
            throw new Error('Table \'guests\' n\'existe pas dans Supabase. Créez-la d\'abord !');
          } else if (error.message.includes('Invalid API key')) {
            throw new Error('Clés Supabase invalides. Vérifiez votre .env.local');
          } else {
